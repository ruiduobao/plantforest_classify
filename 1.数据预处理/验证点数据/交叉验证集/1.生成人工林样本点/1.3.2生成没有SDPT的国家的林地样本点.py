...
功能说明：
本脚本针对没有SDPT数据覆盖的国家，基于指定的范围矢量文件，在每个面中生成林地样本点：
1. 从土地覆盖数据中筛选森林像素（值为2）
2. 在每个范围面中按指定采样率生成样本点（每200个像素选择1个点）
3. 使用PlantTree人工林产品数据进行二次筛选，保留人工林像素（值为2）的点
4. 输出原始林地样本点和经过PlantTree筛选后的人工林样本点

优化特性：
- 基于范围矢量的分面处理，使用边界框读取栅格数据，避免复杂的几何裁剪操作
- 多进程并行处理，充分利用CPU资源
- 分块读取栅格数据，减少内存占用
- 实时内存监控，动态调整处理策略
- 分批保存结果，避免大数据集内存问题
...
from rasterio.mask import mask  # 导入mask用于按矢量裁剪栅格
...
# 范围矢量文件路径（指定需要生成样本点的区域）
BOUNDARY_FILE = r"D:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\1.人工林样本点\3.根据栅格数据进行补充\补充样本点的范围.shp"  # 指定范围矢量
...
# 采样率（每多少个像素选择1个点）
SAMPLING_RATE = 1000  # 每200个森林像素选择1个点
# 进程数量（根据机器情况调整，避免过高导致内存压力）
NUM_PROCESSES = min(6, cpu_count())  # 设置最多6个进程，兼顾性能与稳定性
# 分块处理大小（栅格读取的分块大小）
CHUNK_SIZE = 2000  # 分块大小，平衡内存使用和处理效率
# 分批保存的样本点数量
BATCH_SIZE = 50000  # 每批保存的样本点数量
# 内存使用率阈值（百分比）
MEMORY_THRESHOLD = 75  # 内存使用率超过75%时触发分批保存
# PlantTree采样的分块大小（在worker内一次采样的点数）
PLANTTREE_CHUNK_SIZE = 100000  # 单次采样的点数上限，平衡IO与内存
...
# ===================== 分面处理函数 =====================
def process_polygon_chunk(args):  # 定义处理单个面的函数（用于多进程）
    """处理单个范围面，基于边界框读取栅格数据并生成林地样本点"""  # 函数说明
    poly_id, polygon_geom, landcover_path, forest_value, sampling_rate, target_crs = args  # 解包参数
    
    try:  # 开始异常捕获
        # 打开土地覆盖栅格
        with rasterio.open(landcover_path) as src:  # 打开土地覆盖栅格
            # 确保几何对象与栅格坐标系一致
            if target_crs != src.crs:  # 检查坐标系是否一致
                # 这里假设polygon_geom已经在调用前转换为正确的坐标系
                pass
            
            # 获取矢量面的边界框
            bounds = polygon_geom.bounds  # 获取几何对象的边界框 (minx, miny, maxx, maxy)
            ...
            try:  # 尝试转换坐标
                ...
                if window.width <= 0 or window.height <= 0:  # 如果窗口无效
                    logging.info(f"面 {poly_id}: 边界框超出栅格范围")  # 记录信息
                    return []  # 返回空列表
                
                # 读取窗口内的栅格数据
                bbox_data = src.read(1, window=window)  # 读取第一个波段的窗口数据
                bbox_transform = rasterio.windows.transform(window, src.transform)  # 获取窗口的仿射变换
                
            except Exception as e:  # 捕获坐标转换异常
                logging.warning(f"面 {poly_id} 边界框处理失败: {str(e)}")  # 记录警告
                return []  # 返回空列表
            
            # 筛选森林像素
            valid_mask = (bbox_data == forest_value) & (bbox_data != src.nodata) & (~np.isnan(bbox_data))  # 创建有效像素掩膜
            
            if not np.any(valid_mask):  # 如果没有有效像素
                logging.info(f"面 {poly_id}: 边界框内无森林像素")  # 记录信息
                return []  # 返回空列表
            
            # 获取有效像素的行列索引（相对于窗口）
            rows, cols = np.where(valid_mask)  # 获取有效像素的行列坐标
            values = bbox_data[rows, cols]  # 获取对应的像素值
            ...
            if sample_size > 0:  # 如果需要采样
                ...
                for i in range(len(sample_rows)):  # 遍历每个采样点
                    ...
                    x, y = rasterio.transform.xy(bbox_transform, sample_rows[i], sample_cols[i])  # 转换为地理坐标
                    points.append({
                        'geometry': Point(x, y),  # 点几何
                        'landcover_value': int(sample_values[i]),  # 土地覆盖值
                        'polygon_id': poly_id,  # 所属面的ID
                        'row': int(sample_rows[i]),  # 栅格行坐标（窗口内）
                        'col': int(sample_cols[i])  # 栅格列坐标（窗口内）
                    })
                
                logging.info(f"面 {poly_id}: 边界框内处理了 {total_pixels} 个森林像素，采样了 {len(points)} 个点")  # 记录处理结果
                return points  # 返回采样点列表
            
            return []  # 如果不需要采样，返回空列表
            
    except Exception as e:  # 捕获异常
        logging.error(f"处理面 {poly_id} 时出错: {str(e)}")  # 记录错误日志
        return []  # 返回空列表
...
def generate_forest_sample_points():  # 定义林地样本点生成函数
    ...
    # 读取范围矢量数据
    boundary_gdf = gpd.read_file(BOUNDARY_FILE)  # 读取范围矢量
    logging.info(f"已读取范围矢量，共 {len(boundary_gdf)} 个面，CRS: {boundary_gdf.crs}")  # 记录矢量信息
    ...
    # 坐标系一致性处理
    target_crs = landcover_info['crs']  # 使用栅格的坐标系作为目标坐标系
    if boundary_gdf.crs != target_crs:  # 如果矢量坐标系与栅格不一致
        boundary_gdf = boundary_gdf.to_crs(target_crs)  # 转换矢量坐标系
        logging.info(f"范围矢量已重投影到 {target_crs}")  # 记录重投影
    
    # 准备多进程任务
    tasks = []  # 初始化任务列表
    for idx, row in boundary_gdf.iterrows():  # 遍历每个面
        task = (
            idx,  # 面的索引作为ID
            row.geometry,  # 面的几何
    ...
    logging.info(f"总共创建了 {len(tasks)} 个处理任务（每个面一个任务）")  # 记录任务数量
    ...
    # 使用进度条显示处理进度
    with tqdm(total=len(tasks), desc="处理进度", unit="面") as pbar:  # 创建进度条
        # 多进程处理
        with Pool(processes=NUM_PROCESSES) as pool:  # 创建进程池
            ...
            for i in range(0, len(tasks), batch_size):  # 分批处理
                ...
                # 处理当前批次
                results = pool.map(process_polygon_chunk, batch_tasks)  # 并行处理批次任务
                ...
                if (len(all_points) >= BATCH_SIZE or 
                    ...
                    if all_points:  # 如果有点数据
                        ...
                        batch_file = save_batch_results(
                            all_points, OUTPUT_DIR, batch_num, target_crs, "forest"
                        )  # 保存批次
...
def save_results_and_report(forest_gdf, plantation_gdf, forest_file, output_dir):  # 定义结果保存和报告生成函数
    ...
    with open(report_file, 'w', encoding='utf-8') as f:  # 打开报告文件
        ...
        f.write(f"  范围矢量数据: {BOUNDARY_FILE}\n")  # 写入范围矢量数据路径
...
def main():  # 定义主函数
    ...
    logging.info(f"范围矢量数据: {BOUNDARY_FILE}")  # 记录范围矢量数据路径
