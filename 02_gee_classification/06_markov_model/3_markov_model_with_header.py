...
def vectorized_viterbi_batch(observations_batch, transition_matrix, emission_matrix, initial_probs):
    """
    向量化的维特比算法批量处理版本（修复数值稳定性问题）
    使用Numba JIT编译加速，采用对数概率防止数值下溢
    ...
    # 转换为对数概率以防止数值下溢
    log_transition = np.log(np.maximum(transition_matrix, 1e-10))
    log_emission = np.log(np.maximum(emission_matrix, 1e-10))
    log_initial = np.log(np.maximum(initial_probs, 1e-10))
    ...
    for batch_idx in prange(batch_size):
        ...
        # 维特比表格（使用对数概率）
        log_viterbi_table = np.full((n_states, valid_T), -np.inf)
        path_table = np.zeros((n_states, valid_T), dtype=np.int32)
        
        # 初始化第一个时间步 - 修复：将观测值转换为0-based索引
        obs_0 = valid_obs[0] - 1  # 将1,2,3转换为0,1,2
        if 0 <= obs_0 < n_states:  # 确保索引有效
            for s in range(n_states):
                log_viterbi_table[s, 0] = log_initial[s] + log_emission[s, obs_0]
        
        # 前向传播
        for t in range(1, valid_T):
            obs_t = valid_obs[t] - 1  # 将1,2,3转换为0,1,2
            if 0 <= obs_t < n_states:  # 确保索引有效
                for s in range(n_states):
                    # 计算转移概率（对数空间）
                    max_log_prob = -np.inf
                    best_prev = 0
                    for prev_s in range(n_states):
                        log_prob = log_viterbi_table[prev_s, t-1] + log_transition[prev_s, s]
                        if log_prob > max_log_prob:
                            max_log_prob = log_prob
                            best_prev = prev_s
                    
                    log_viterbi_table[s, t] = max_log_prob + log_emission[s, obs_t]
        ...
        best_last_state = 0
        for s in range(n_states):
            if log_viterbi_table[s, valid_T-1] > max_log_prob:
                max_log_prob = log_viterbi_table[s, valid_T-1]
                best_last_state = s
...
class FastLandCoverHMMOptimizer:
    ...
    def load_raster_data(self, file_paths, years=None):
        """加载栅格数据，并应用序列填充（Sequence Padding）"""
        if not file_paths:
            print("警告: 未提供输入文件。")
            self.input_files = []
            self.years = []
            self.original_years = []  # 新增
            return

        original_files = list(file_paths)
        original_years = list(years) or list(range(len(original_files)))
        
        # 新增：保存原始年份列表，用于后续输出
        self.original_years = original_years

        # --- 新增：序列填充逻辑 ---
        print("应用序列填充以优化边缘年份结果...")
        
        # 1. 复制头部
        padded_files = [original_files[0]] + original_files
        padded_years = [original_years[0] - 1] + original_years  # 人为创建一个"前一年"
        
        # 2. 复制尾部
        padded_files.append(original_files[-1])
        padded_years.append(original_years[-1] + 1)  # 人为创建一个"后一年"
        
        self.input_files = padded_files
        self.years = padded_years
        # --- 填充逻辑结束 ---

        print(f"加载 {len(self.input_files)} 个栅格文件 (其中2个为填充文件):")
        for i, (file_path, year) in enumerate(zip(self.input_files, self.years)):
            pad_info = ""
            if i == 0 or i == len(self.input_files) - 1:
                pad_info = " (填充)"
            print(f"  {i+1}. {year}{pad_info}: {os.path.basename(file_path)}")
    
    def optimize_raster_time_series(self,Zone="zone7", chunk_size=1500):
        """
        ...
        # 创建输出文件
        output_files = []
        # 修改：使用 self.original_years 来确保只为真实年份创建输出文件
        for year in self.original_years:
            output_file = os.path.join(self.output_dir, f'optimized_{Zone}_{year}.tif')
                ...
                for temp_file, task in zip(temp_chunk_files, tasks):
                    ...
                    try:
                        ...
                        for t, dst in enumerate(output_datasets):
                            # temp_data 包含了 N+2 个时间层
                            # 第 t 个输出文件 (t从0开始) 对应原始的第 t 年
                            # 它在填充数据中的索引是 t+1
                            chunk_data = temp_data[t+1].astype(np.uint8)  # 修改：跳过填充的第一个时间层
            ...
            with tqdm(total=len(tasks), desc="多进程处理") as pbar:
                for future in as_completed(future_to_task):
                    ...
                    try:
                        ...
                        # 写入结果
                        for t, dst in enumerate(output_datasets):
                            # optimized_chunk 包含 N+2 个时间层，我们需要写入索引 1 到 N+0
                            dst.write(optimized_chunk[t+1], 1, window=window)  # 修改
    ...
    def _optimize_chunk_fast(self, time_series_chunk):
        ...
        # 数据验证
        if n_times == 0 or height == 0 or width == 0:
            print(f"警告：数据块尺寸无效 {n_times}x{height}x{width}")
            return time_series_chunk
        
        # 检查数据范围
        unique_values = np.unique(time_series_chunk)
        print(f"数据块包含的值: {unique_values}")
        ...
        for start_idx in range(0, n_pixels, self.batch_size):
            ...
            try:
                ...
                # 验证输出
                if optimized_batch is not None and optimized_batch.shape == batch.shape:
                    optimized_sequences[start_idx:end_idx] = optimized_batch
                    processed_pixels += batch.shape[0]
                else:
                    print(f"警告：批次 {start_idx}-{end_idx} 优化失败，使用原始数据")
                    optimized_sequences[start_idx:end_idx] = batch
                    failed_pixels += batch.shape[0]
                    
            except Exception as e:
                print(f"批次 {start_idx}-{end_idx} 处理异常: {e}")
                optimized_sequences[start_idx:end_idx] = batch
                failed_pixels += batch.shape[0]
        
        print(f"处理统计: 成功 {processed_pixels} 像元, 失败 {failed_pixels} 像元")
        
        # 重塑回原始格式
        optimized_chunk = optimized_sequences.reshape(height, width, n_times).transpose(2, 0, 1)
        
        # 验证输出格式
        if optimized_chunk.shape != time_series_chunk.shape:
            print(f"错误：输出格式不匹配 {optimized_chunk.shape} != {time_series_chunk.shape}")
            return time_series_chunk
...
def main():
    ...
    # 数据配置
    Zone="zone7"
    BASE_DATA_DIR = r"D:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\2.GEE导出结果_结果合并\zone7\\"
    YEARS = list(range(2017, 2025))
    
    FILE_PATTERN = Zone+"_{}.tif"
    
    # 输出目录
    output_dir = r"D:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\3.GEE导出结果_结果合并_马尔可夫模型\\"+Zone +"首尾填充"
    ...
    transition_matrix_2017 = np.array([
        [0.94,   1e-9,   0.06],
        [0.02,  0.96,   0.02],
        [0.045,   0.01,   0.945]
    ])
    ...
    if input_files:
        ...
        # 执行优化
        optimized_files = optimizer.optimize_raster_time_series(chunk_size=CHUNK_SIZE,Zone=Zone)
