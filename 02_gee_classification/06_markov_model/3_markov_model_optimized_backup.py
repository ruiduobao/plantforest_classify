...
def main():
    ...
    # 性能配置（优化后）
    N_PROCESSES = 4  # 减少进程数以降低资源竞争
    USE_GPU = GPU_AVAILABLE  # 如果可用则使用GPU
    BATCH_SIZE = 1000  # 减小批处理大小以降低内存压力
    CHUNK_SIZE = 2000  # 适中的块大小平衡效率和内存
    
    # 数据配置
    BASE_DATA_DIR = r"D:\地理所\论文\东南亚10m人工林提取\数据\第二次分类\合并"
    YEARS = list(range(2017, 2025))
    FILE_PATTERN = "merged_zone5_{}_*.tif"
    
    # 输出目录
    output_dir = r"D:\地理所\论文\东南亚10m人工林提取\数据\第二次分类_马尔可夫模型_高性能"
