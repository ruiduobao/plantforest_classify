...
def clip_worker(file_path, clip_shp_path, intermediate_dir):
    """
    多进程裁剪的工作函数，负责裁剪单个文件。
    
    Args:
        file_path (str): 输入栅格文件路径。
        clip_shp_path (str): 裁剪矢量文件路径。
        intermediate_dir (str): 中间文件存储目录。
        
    Returns:
        str or None: 成功则返回裁剪后文件的路径，失败则返回None。
    """
    try:
        # 构造输出文件路径
        base_name = os.path.basename(file_path)
        clipped_file_path = os.path.join(intermediate_dir, f"{os.path.splitext(base_name)[0]}_clipped.tif")
        
        # 如果文件已存在，则跳过，避免重复处理
        if os.path.exists(clipped_file_path):
            logging.info(f"文件已存在，跳过裁剪: {clipped_file_path}")
            return clipped_file_path

        # 调用gdal.Warp进行裁剪
        logging.info(f"进程 {os.getpid()} 开始裁剪: {file_path}")
        clip_success = clip_raster_by_shp(file_path, clip_shp_path, clipped_file_path)
        
        if clip_success:
            logging.info(f"进程 {os.getpid()} 裁剪成功: {clipped_file_path}")
            return clipped_file_path
        else:
            logging.warning(f"进程 {os.getpid()} 裁剪失败，跳过: {file_path}")
            return None
    except Exception as e:
        # 捕获未知异常，确保进程不会崩溃
        logging.error(f"裁剪工作进程 {os.getpid()} 中发生严重错误: {file_path}, 原因: {e}")
        return None

def clip_and_mosaic_one_year(input_root, output_dir, year, clip_shp_path, intermediate_dir):
    """
    对指定年份进行裁剪和跨zone的镶嵌合并（多进程优化版）
    该函数首先使用多进程并行裁剪所有栅格，然后对裁剪后的结果进行镶嵌。
    """
    # 构造最终输出文件名
    output_file = os.path.join(
        output_dir,
        f"人工林提取结果_每个区域16000个样本点_{year}年_变化概率_筛选样本_逐年分类_裁剪.tif"
    )
    
    # 查找需要处理的原始文件
    files_to_process = find_year_zone_files(input_root, year)
    if not files_to_process:
        logging.error(f"年份 {year} 未找到任何用于镶嵌的tif文件")
    ...
    # --- 多进程裁剪 ---
    logging.info(f"开始对年份 {year} 的 {len(files_to_process)} 个文件进行多进程裁剪...")
    
    # 准备任务参数列表，每个元组包含一个文件的所有处理参数
    tasks = [(file_path, clip_shp_path, intermediate_dir) for file_path in files_to_process]
    
    # 设置进程数，保守起见，留一个核心给操作系统使用
    num_processes = max(1, os.cpu_count() - 7 if os.cpu_count() else 7)
    logging.info(f"使用 {num_processes} 个CPU核心进行并行处理...")

    clipped_files = []
    # 使用进程池执行并行裁剪
    # 使用 with 语句确保进程池被正确关闭
    with multiprocessing.Pool(processes=num_processes) as pool:
        # starmap可以向工作函数传递多个参数
        results = pool.starmap(clip_worker, tasks)
    
    # 从结果中过滤掉失败的任务（worker返回None）
    clipped_files = [res for res in results if res]

    # 检查裁剪结果
    if not clipped_files:
        logging.error(f"年份 {year} 所有文件裁剪失败或未找到")
        return False, output_file
    
    if len(clipped_files) < len(files_to_process):
        logging.warning(f"年份 {year} 有 {len(files_to_process) - len(clipped_files)} 个文件裁剪失败。")

    logging.info(f"年份 {year} 裁剪完成，成功处理 {len(clipped_files)} 个文件。")
    # --- 多进程裁剪结束 ---

    # 开始镶嵌裁剪后的文件
    logging.info(f"开始年份 {year} 的镶嵌，使用 {len(clipped_files)} 个裁剪后的文件")
    ok = create_unified_mosaic(clipped_files, output_file)
    
    return ok, output_file

def clip_raster_by_shp(input_raster, clip_shp, output_raster):
    """
    使用gdal.Warp按矢量边界裁剪栅格
    """
    try:
        from osgeo import gdal
        
        logging.info(f"输出裁剪文件到: {output_raster}")
        ...
        if ds is None:
            logging.error("gdal.Warp 裁剪失败，返回空数据集")
            return False
        
        ds = None # 关闭数据集
        
        if not os.path.exists(output_raster):
            logging.error(f"裁剪后的输出文件未创建: {output_raster}")
            return False
            
        logging.info(f"文件裁剪成功: {output_raster}")
        return True

    except ImportError:
        logging.error("GDAL未安装，无法执行裁剪操作")
        return False
    except Exception as e:
        logging.error(f"裁剪栅格时发生错误: {e}")
        return False
...
def main():
    ...
    output_dir = r"F:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\4.GEE导出结果_结果合并_马尔可夫模型_逐年合并"  # 输出目录（逐年合并）
    clip_shp_path = r"F:\地理所\论文\东南亚10m\数据\裁剪矢量\东南亚裁剪矢量.shp" # 裁剪矢量
    intermediate_dir = r"F:\地理所\论文\东南亚10m\数据\正式分类_10.29\3.GEE导出结果_结果合并_马尔可夫模型_中间过程" # 中间文件目录
    ...
    logging.info(f"中间文件目录: {intermediate_dir}")
    ...
    try:
        for y in years:  # 遍历每个年份
            ...
            ok, out_path = clip_and_mosaic_one_year(input_dir, output_dir, y, clip_shp_path, intermediate_dir)  # 执行该年份的镶嵌
