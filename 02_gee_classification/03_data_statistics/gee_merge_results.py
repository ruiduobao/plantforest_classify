...
def main():
    ...
    """
    # 定义路径
    input_dir = r"D:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\GEE导出结果\已处理完"
    output_dir = r"D:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\GEE导出结果\GEE下载_逐年镶嵌_按区域镶嵌"
    output_file = os.path.join(output_dir, "人工林提取结果_每个区域16000个样本点_ZONE5_2023年_变化概率_筛选样本_逐年分类.tif")
    ...
    try:
        ...
        # 根据文件数量选择处理策略
        if len(tif_files) > 20:
            logging.info("文件数量较多，使用分批处理策略")
            success = create_batch_mosaic(tif_files, output_file, batch_size=10)
        else:
            logging.info("文件数量适中，使用直接镶嵌")
