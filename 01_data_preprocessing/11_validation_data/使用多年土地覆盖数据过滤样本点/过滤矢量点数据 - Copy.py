...
SHP_PATH = r"K:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\2.自然林样本点\5.按照网格筛选1000个点"
# 为了获取稳定的样本数据，过滤每一年的矢量点数据，只保留这些数据人工林、自然林分别在2017年-2024年中，矢量点值要这些年一直为2，过滤其他OTHERS点矢量，几年不能有一个值为2
SHP_FILE =[
"Zone5_2024_Merged_AllBand_Sample.gpkg",]
# SHP_FILE =[
...
OUT_PUT_SHP_PATH=r"K:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\2.自然林样本点\5.按照网格筛选1000个点_再经过8年的土地覆盖数据筛选"
...
def filter_sample_points(chunk_size=10000):
    ...
    output_file = os.path.join(OUT_PUT_SHP_PATH, "natural_forest_points_sampled_AfterCDD_AfterLandcoverFilter.shp")
