...
# -*- coding: utf-8 -*-
"""
自然林样本点筛选脚本 - 基于人工林产品和SDPT数据的双重筛选
=================================================================

功能说明：
本脚本对林地样本点进行双重筛选，确保最终的样本点位于自然林区域：
1. PlantTree筛选：保留在人工林30米产品范围外的点（排除人工林区域）
2. SDPT筛选：保留在SDPT（人工林种植园）范围外的点（排除种植园区域）

优化特性：
- 多进程并行处理，提升大数据集处理效率
- 基于网格的空间索引优化，显著提升SDPT筛选性能
- 分块处理机制，有效控制内存使用
- 详细的进度监控和日志记录
- 批量保存和统计报告生成

输入数据：
- 林地样本点数据（来自上一步生成的林地样本点）
...
INPUT_DIR = r"D:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\3.自然林样本点\1.林地样本点"  # 指定上一步的输出目录
# 人工林30米产品栅格文件路径
...
# 输出目录路径
OUTPUT_DIR = r"D:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\3.自然林样本点\2.筛选自然林样本点"  # 指定输出目录
# 进程数量（根据机器情况调整，避免过高导致内存压力）
NUM_PROCESSES = min(8, cpu_count())  # 设置最多8个进程，兼顾性能与稳定性
# 点数据分块大小（点数量大，分块处理更稳健）
POINT_CHUNK_SIZE = 500_000  # 每次处理20万点，避免一次性占用过多内存
# 采样栅格的分块大小（在worker内一次采样的点数）
SAMPLE_CHUNK_SIZE = 200_000  # 单次采样的点数上限，平衡IO与内存
# PlantTree产品中"表示非林地"的像素判断规则（NoData表示非林地）
# 人工林产品数据编码：1=自然林，2=人工林，NoData=非林地
# 对于自然林样本点筛选，只保留值为1的点（自然林）
...
def setup_logging(output_dir):  # 定义日志设置函数
    ...
    log_file = os.path.join(output_dir, f"natural_forest_point_filter_{timestamp}.log")  # 构造日志文件路径
...
def find_latest_input_file(input_dir):  # 定义查找最新输入GPKG的函数
    ...
    for name in os.listdir(input_dir):  # 遍历目录中文件名
        if name.lower().endswith('.gpkg') and 'forest_sample_points_' in name:  # 筛选匹配命名模式的gpkg
            candidates.append(os.path.join(input_dir, name))  # 将符合条件的文件加入列表
    if not candidates:  # 如果没有候选文件
        raise FileNotFoundError("输入目录中未找到林地样本点GPKG文件 (forest_sample_points_*.gpkg)")  # 抛出异常
        ...
        with rasterio.open(raster_path) as src:  # 在子进程内打开栅格，避免句柄跨进程问题
            ...
            for i in range(0, len(coords), SAMPLE_CHUNK_SIZE):  # 采用子分块采样，避免一次性读太多
                ...
                for val in values:  # 遍历每个采样结果
                    ...
                    # 对于自然林样本点筛选，只保留PlantTree值为1（自然林）的点
                    # 剔除值为2（人工林）和NoData值的点
...
def main():  # 定义主函数
    ...
    logging.info(f"PlantTree筛选后保留(值为1的自然林): {kept_by_plant}, 剔除(值为2的人工林和NoData): {dropped_by_plant}")  # 输出筛选统计
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # 生成时间戳
    output_file = os.path.join(OUTPUT_DIR, f"natural_forest_points_selected_{timestamp}.gpkg")  # 构造输出文件名
    ...
    # 生成统计报告
    report_path = os.path.join(OUTPUT_DIR, f"natural_forest_point_filter_report_{timestamp}.txt")  # 报告文件路径
    with open(report_path, 'w', encoding='utf-8') as f:  # 打开报告文件进行写入
        ...
        f.write(f"PlantTree筛选后保留(人工林范围外): {kept_by_plant}, 剔除(人工林范围内): {dropped_by_plant}\n")  # 写入PlantTree筛选统计
