...
"""
非林地样本点生成脚本
根据土地覆盖数据生成非林地样本点（值非2且不是空值）
每50个像素随机选择一个像素为样本点，生成GPKG点数据
...
OUTPUT_DIR = r"D:\地理所\论文\东南亚10m人工林提取\数据\验证点数据\交叉验证集\2.非林地样本点\1.土地覆盖数据生成"
FOREST_VALUE = 2  # 森林像素值
SAMPLING_RATE = 1000  # 每100个像素选择1个
...
def setup_logging(output_dir):
    ...
    log_file = os.path.join(output_dir, f"nonforest_sampling_log_{timestamp}.txt")
...
def process_chunk(args):
    ...
    try:
        ...
        if sample_size > 0:
            ...
            logging.info(f"块 {chunk_id}: 处理了 {total_pixels} 个有效像素，采样了 {len(points)} 个点")
            return points
...
def generate_nonforest_sample_points():
    """
    生成非林地样本点（优化版本，支持进度显示和分批保存）
    """
    logging.info("开始生成非林地样本点...")
...
def save_results(gdf, output_dir):
    ...
    # 生成统计报告
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_file = os.path.join(output_dir, f"nonforest_sampling_report_{timestamp}.txt")
