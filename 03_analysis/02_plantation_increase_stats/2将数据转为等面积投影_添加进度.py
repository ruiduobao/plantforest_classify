...
# 输出TIF文件路径
TIF_OUT_PATH = r"F:\人工林增长和自然林消失\output\plantation_growth_and_natural_forest_disappearance_等面积投影3.tif"
...
def reproject_to_equal_area(input_file_path, output_file_path):
    ...
    try:
        ...
        gdal_cache_size = 16384
        logging.info(f"设置GDAL缓存大小为: {gdal_cache_size} MB")
        
        with rasterio.Env(GDAL_CACHEMAX=gdal_cache_size):
            with rasterio.open(input_file_path) as src:
                ...
                # 使用 WarpedVRT 进行虚拟重投影
                with rasterio.vrt.WarpedVRT(src, **kwargs) as vrt:
                    with rasterio.open(output_file_path, 'w', **kwargs) as dst:
                        # 计算总的块数以用于进度条
                        total_blocks = math.ceil(vrt.width / kwargs['blockxsize']) * math.ceil(vrt.height / kwargs['blockysize'])
