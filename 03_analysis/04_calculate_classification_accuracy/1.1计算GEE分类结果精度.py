...
# 设置分类结果tif的根目录
CLASSIFICATION_ROOT = r"D:\地理所\论文\东南亚10m人工林提取\数据\正式分类_10.29\2.GEE导出结果_结果合并"  # 分类结果根目录
...
OUTPUT_DIR = os.path.join(VALIDATION_ROOT, "precision_results")  # 每个zone每年结果与年度汇总的输出目录
...
def zone_year_tif_path(zone: int, year: int) -> str:
    ...
    tif_name = f"zone{zone}_{year}.tif"  # 生成tif文件名
    # 返回完整路径
